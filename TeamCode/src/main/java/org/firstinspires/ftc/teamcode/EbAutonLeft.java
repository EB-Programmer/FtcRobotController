...
@Autonomous(name = "EbAutonLeft", group = "Robot")
public class EbAutonLeft extends LinearOpMode {
    ...
    private void driveToChamberPosition() {
        drive(DriveDirection.FORWARD, 37.5, DRIVE_POWER);
    }
    ...
    private void driveToYellowSamples() {
        drive(DriveDirection.BACKWARD,8,DRIVE_POWER);
        drive(DriveDirection.STRAFE_LEFT, 35, DRIVE_POWER);
        drive(DriveDirection.FORWARD, 28, DRIVE_POWER);
    }

    private void pushYellowSamplesToScoringArea() {
        drive(DriveDirection.STRAFE_LEFT, 15, DRIVE_POWER);
        drive(DriveDirection.BACKWARD, 55, DRIVE_POWER);
        drive(DriveDirection.STRAFE_RIGHT, 4, DRIVE_POWER);
        drive(DriveDirection.FORWARD, 58, DRIVE_POWER);
        drive(DriveDirection.STRAFE_LEFT, 16, DRIVE_POWER);
        drive(DriveDirection.BACKWARD, 55, DRIVE_POWER);
        drive(DriveDirection.FORWARD, 55, DRIVE_POWER);
    }

    private void driveToParkPosition() {
        drive(DriveDirection.STRAFE_RIGHT, 37, DRIVE_POWER);
    }
    ...
    private void drive(DriveDirection direction, double inches, double power) {
        switch(direction) {
            case FORWARD:
                drive(inches, DriveMethod.DEFAULT_FORWARD, power);
                break;
            case BACKWARD:
                drive(-inches, DriveMethod.DEFAULT_FORWARD, power);
                break;
            case STRAFE_LEFT:
                drive(-inches, DriveMethod.STRAFE, power);
                break;
            case STRAFE_RIGHT:
                drive(inches, DriveMethod.STRAFE, power);
                break;
    ...
    private void turn(DriveDirection direction, double degrees, double power) {
        final double TICKS_TO_DEGREES = 100;

        switch(direction) {
            case TURN_LEFT:
                drive(-degrees*TICKS_TO_DEGREES, DriveMethod.TURN, power);
                break;
            case TURN_RIGHT:
                drive(degrees*TICKS_TO_DEGREES, DriveMethod.TURN, power);
                break;
